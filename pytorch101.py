...
def multiples_of_ten(start, stop):
  ...
  if start%10 == 0:
    ...
    a = (((start//10)+1)*10)
  if stop-a >= 10:
    x = torch.arange(a, stop, 10, dtype=torch.float64)
  else:
...
def slice_assignment_practice(x):
  ...
  x[2:-1,: 6] =  torch.tensor([3, 4, 3, 4, 5, 5])
  # x[:2, :2] = torch.tensor([[2, 3],[2,3]])
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def make_one_hot(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y = torch.zeros(len(x), len(x), dtype=torch.int32)
  y[x] = 1
