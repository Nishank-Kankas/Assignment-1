...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range (0, num_train):
    for j in range (0, num_test):
      dists[i][j] = torch.sqrt(torch.sum(torch.square(train_flat[i]-test_flat[j])))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range (0, num_train):
    dists[i] = torch.sqrt(torch.sum(torch.square(test_flat - train_flat[i]), axis=1))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = torch.sqrt(torch.sum(torch.square(train_flat.view(num_train, 1, -1) - test_flat), axis=2))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  indx = (torch.argsort(dists, dim = 0).t())[:,:k]
  y_labols = y_train[indx.reshape(-1)].reshape(-1,k)
  y_pred = torch.mode(y_labols, dim = 1)[0]
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for j in range (0,num_folds):
      ...
      x_train_temp = [x for x in x_train_folds]
      y_train_temp = [y for y in y_train_folds]
      del x_train_temp[j]
      del y_train_temp[j]
      x_train = torch.cat(x_train_temp, dim=0)
      y_train = torch.cat(y_train_temp, dim=0)
      a = KnnClassifier(x_train, y_train)
      temp_list.append(a.check_accuracy(x_test, y_test,k))
    k_to_accuracies[k] = temp_list; 
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k, accs in sorted(k_to_accuracies.items()):
    if (sum(accs) / len(accs)) > best_accuracy:
      best_accuracy = sum(accs) / len(accs)
      best_k = k
